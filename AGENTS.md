...
Use `dune build @install` to build the library and installable executable. Run `dune runtest` to execute the OUnit suite. `make build`, `make test`, and `make format` are thin wrappers around the main dune and formatting commands. Use `dune install` and `dune uninstall` for local installs, and `dune clean` to remove `_build/`.
