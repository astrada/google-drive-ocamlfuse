...
Use `dune build @install` to build the library and installable executable. Run `dune runtest` to execute the OUnit suite. Run build and test *sequentially* because `dune` uses a lock file that prevents running more than one `dune` command in parallel. `make build`, `make test`, and `make format` are thin wrappers around the main dune and formatting commands. Use `dune install` and `dune uninstall` for local installs, and `dune clean` to remove `_build/`.
